...
void connect_direct(char * path, char* to_eval) ;
void connect_ssh(char *host, char *path, char* to_eval) ;
...
void usage(char* argv0, char* bad_argument) {
    if (bad_argument != NULL) {
        fprintf(stderr, "%s: unrecognized arguments: %s\n", argv0, bad_argument);
    }
    fprintf(stderr, "usage: %s /path/to/socket [ --eval CODE ]\n", argv0);
    fprintf(stderr, "       %s remote_user@remote_host:/path/to/socket [ --eval CODE ]\n", argv0);
    exit(1);
}
...
main(int argc,char *argv[], char *envp[]) {
    ...
    for(next_arg=2;next_arg<argc;next_arg++) {
	if(!strncmp("--eval",argv[next_arg],6)) {
            if (next_arg == argc - 1) {
                usage(argv[0], NULL);
                exit(1);
            }
            to_eval = argv[++next_arg];
            continue; 
        }
	...
	*path='\0';
	...
	connect_ssh(host,path, to_eval);
    } else {
	logprintf("attachtty","connecting directly to %s\n",path);
	connect_direct(path, to_eval);
    }
}
...
void connect_direct(char * path, char* to_eval) {
    ...
    if (to_eval != NULL) {
        write(sock, to_eval, strlen(to_eval));
        write(sock, "\n", 1);
    } else {
        while(! time_to_die) {
            if(was_interrupted) {
...
void connect_ssh(char *host, char *path, char* to_eval) {
	...
	logprintf("attachtty","Successfully started"); 
        if (to_eval != NULL) {
            write(pipe_des[1], to_eval, strlen(to_eval));
            write(pipe_des[1], "\n", 1);
        } else {
            while(! time_to_die) {
                if(was_interrupted) {
